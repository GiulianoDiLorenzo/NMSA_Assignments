function [fluxes] = setFluxes(rho_max, u_max, plotMe)
    ...
    if plotMe
        ...
        subplot(2,1,1);
            ...
            yticklabels([string(yt(1:end-1)), '$f_2^{\max}$']); % Replace last tick with LaTeX
        ...
        subplot(2,1,2)
            ...
            hold on;
            plot(rho, df2(rho), 'Color','r')
            
            scatter(rho_c_f1, df1(rho_c_f1), 50, 'b', 'filled') % Red marker for first flux
            scatter(rho_c_f2, df2(rho_c_f2), 50, 'r', 'filled') % Green marker for second flux
